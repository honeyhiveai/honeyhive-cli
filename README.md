...
Set the `HH_PROJECT_API_KEY` environment variable. The CLI reads it automatically when no `--project-api-key` flag is provided:
...
> **Deprecated aliases:** the `--api-key` flag and the `HH_API_KEY` environment variable are still accepted but will be removed in the next major version. The `--api-key` flag logs a deprecation warning to stderr; the `HH_API_KEY` environment variable logs one via the SDK. Migrate to `--project-api-key` / `HH_PROJECT_API_KEY`.
...
> **Deprecated aliases:** the `--base-url` flag and the `HH_API_URL` environment variable are still accepted but will be removed in the next major version. The `--base-url` flag logs a deprecation warning to stderr; the `HH_API_URL` environment variable logs one via the SDK. Migrate to `--data-plane-url` / `HH_DATA_PLANE_URL`.

## Verbose logging

Pass `--verbose` (or set `HH_VERBOSE=true`) to log the resolved data plane URL, a masked API key, and the CLI version on startup. Useful when debugging "is this hitting prod or staging?" or "did the right `HH_PROJECT_API_KEY` get picked up?".
...
Output is written to stderr and only fires once per invocation. An explicit `--verbose` flag matches the precedence of the other flags. The API key is masked so only a recognized key prefix and the last 4 characters are shown; anything else is replaced with asterisks.
