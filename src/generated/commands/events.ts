...
export function eventsCommand(): Command {
  ...
  cmd
    ...
    .requiredOption(
      '--event <json>',
      'Full event object for legacy event creation endpoints (required)',
    )
...
`,
  ...
  cmd
    ...
    .requiredOption(
      '--model_event <json>',
      'Model event object with model-specific fields and legacy aliases (required)',
    )
    .action(async (opts: Record<string, unknown>, command: Command) => {
      try {
        const client = createClient(command);
        ...
        } as Parameters<typeof client.events.createModel>[0]);
        if (result !== undefined) {
          process.stdout.write(JSON.stringify(result, null, 2) + '\n');
        }
      } catch (error: unknown) {
...
`,
  ...
  cmd
    ...
    .option(
      '--is_single_session <value>',
      '(deprecated) Legacy field name for single_session (backward compatibility)',
    )
    ...
    .action(async (opts: Record<string, unknown>, command: Command) => {
      try {
        if (opts.is_single_session !== undefined) {
          console.warn(
            'Warning: option "--is_single_session" is deprecated and will be removed in the next major version.',
          );
        }
        if (opts.session !== undefined) {
          console.warn(
            'Warning: option "--session" is deprecated and will be removed in the next major version.',
          );
        }
        const client = createClient(command);
  ...
  cmd
    ...
    .action(async (opts: Record<string, unknown>, command: Command) => {
      try {
        ...
        } as Parameters<typeof client.events.createModelBatch>[0]);
        if (result !== undefined) {
          process.stdout.write(JSON.stringify(result, null, 2) + '\n');
        }
      } catch (error: unknown) {
  ...
  cmd
    ...
    .action(async (opts: Record<string, unknown>, command: Command) => {
      try {
        ...
        } as Parameters<typeof client.events.getEventsSchema>[0]);
        if (result !== undefined) {
          process.stdout.write(JSON.stringify(result, null, 2) + '\n');
        }
      } catch (error: unknown) {
